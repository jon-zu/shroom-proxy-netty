        ...
        for (int j = 0; j < 6; j++) {
            ...
            if (j % 2 == 0) {
                for (int i = 0; i < length; i++) {
                    ...
                    cur = Util.byteRotateLeft(cur, 3);
                    cur += dataLength;
                    cur ^= remember;
                    remember = cur;
                    cur = Util.byteRotateRight(cur, (int) dataLength & 0xFF);
                    cur = ((byte) ((~cur) & 0xFF));
                    cur += 0x48;
                    dataLength--;
                    buf.setByte(m, cur);
                ...
                for (int i = length - 1; i >= 0; i--) {
                    int m = i + offset;
                    byte cur = buf.getByte(m);
                    cur = Util.byteRotateLeft(cur, 4);
                    cur += dataLength;
                    cur ^= remember;
                    remember = cur;
                    cur ^= 0x13;
                    cur = Util.byteRotateRight(cur, 3);
                    dataLength--;
                    buf.setByte(m, cur);
                }
            }
        ...
        for (int j = 1; j <= 6; j++) {
            ...
            if (j % 2 == 0) {
                for (int i = 0; i < length; i++) {
                    ...
                    cur -= 0x48;
                    cur = ((byte) ((~cur) & 0xFF));
                    cur = Util.byteRotateLeft(cur, (int) dataLength & 0xFF);
                    nextRemember = cur;
                    cur ^= remember;
                ...
                for (int i = length - 1; i >= 0; i--) {
                    ...
                    remember = nextRemember;
                    cur -= dataLength;
                    cur = Util.byteRotateRight(cur, 4);
                    buf.setByte(m, cur);
                    dataLength--;
                }
            }
